...
| Commande | Description |
|----------|-------------|
| `akasha doctor` | Vérifie : Rust, Node, binaire daemon, specs, santé du daemon |
...
*Variables optionnelles (debug / avancé)* : `AKASHA_LOG_LLM_RESPONSE=1` (log détaillé des réponses LLM) ; `AKASHA_SPEC_DIR` (chemin vers le dossier `spec` pour la suite d’évals) ; `AKASHA_VAULT_MASTER_KEY` (clé maître pour le vault fichier, si utilisé).

### Fichiers de configuration

- **llm_router.yaml** : recherché dans l'ordre : data_dir, puis racine du projet. Définit les providers (Ollama, OpenAI, OpenRouter) et les modèles par type de tâche.
...
Raccourcis TUI :
- **Tab** : basculer entre Chat, Routeur (métriques) et Doc (documentation)
- **Entrée** : envoyer le message (mode Chat)
- **↑ / ↓, PgUp / PgDn, Home / End** : défilement du contenu (modes Chat et Doc)
- **R** : rafraîchir les métriques (Routeur) ou actualiser la doc (Doc)
...
| `/models` | Liste des modèles (tous les providers configurés : Ollama, OpenAI, OpenRouter, etc.) |
