...
- **État utilisateur (activation / désinstallation)** : le fichier **`data_dir/plugin_state.json`** mémorise les plugins **désactivés manuellement** (indépendamment de la réputation). L’API expose `POST /api/plugins/{id}/disable`, `POST /api/plugins/{id}/enable` et `POST /api/plugins/{id}/uninstall` (supprime le dossier `data_dir/plugins/{id}/`). `GET /api/plugins` inclut `description` et `disabled_reason` (`manual` ou `reputation`).
...
| `formality`  | string          | Non         | Registre d’adresse : `"formal"` ou `"informal"` (ex. vouvoiement vs tutoiement en français). Absent ou `null` = aucune consigne supplémentaire dans le prompt. |
...
Le daemon charge le profil au démarrage (et après chaque POST `/api/agent-profile`). À chaque tour, il produit un bloc `[Agent profile and instructions]` **entièrement en anglais** (texte injecté au modèle) avec : identité (nom depuis le profil ou défaut produit « Akasha », jamais depuis le YAML `personality_core`), pronoms si `gender`, registre d’adresse si `formality`, rôle, texte de personnalité, règles, can_do, cannot_do, puis (si les YAML `spec/personality_*.yaml` sont présents) valeurs, traits, règles comportementales et politique d’initiative — la mission/posture/relation du YAML n’est pas répétée lorsque l’utilisateur a renseigné un texte de personnalité, pour éviter les contradictions avec l’UI. Ce bloc est injecté dans le contexte système avant la mémoire et le RAG. L’avatar n’est pas inclus dans le prompt.
...
| `AKASHA_EMBEDDED_MODEL`                                                   | string         | Modèle Candle : `qwen3_0_6b` (défaut) ou `baguettotron`. Ignoré si backend `llama_cpp` actif.                                                                                                                                                                        |
| `AKASHA_EMBEDDED_BACKEND`                                                 | string         | `auto` (défaut), `llama_cpp`, `candle`, `baguettotron`. `auto` choisit llama-cpp si GGUF présent, sinon Baguettotron si `AKASHA_EMBEDDED_MODEL=baguettotron`, sinon Candle.                                                                                          |
| `AKASHA_EMBEDDED_GGUF_PATH`                                               | chemin         | Chemin absolu vers un fichier `.gguf` (sinon `{data_dir}/models/embedded/default.gguf`).                                                                                                                                                                                |
| `AKASHA_EMBEDDED_N_GPU_LAYERS`                                            | entier         | Couches offload GPU pour llama-cpp (défaut 99 si CUDA compilé).                                                                                                                                                                                                         |
