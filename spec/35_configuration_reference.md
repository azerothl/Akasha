...
| `browser_enabled`             | booléen          | Non (défaut : false)             | Activer l’automation navigateur (outil `browser` : navigate, snapshot). Nécessite Playwright (`npx playwright install chromium`) et runner `scripts/playwright-runner` ou `AKASHA_PLAYWRIGHT_RUNNER`. |
...
- **Automation navigateur (spec 39)** : pour que l'agent utilise l'outil `browser` (navigate, snapshot), définir `browser_enabled: true`, installer Playwright (`npx playwright install chromium`) dans le répertoire du runner (`scripts/playwright-runner` ou chemin défini par `AKASHA_PLAYWRIGHT_RUNNER`), et optionnellement `browser_allowed_domains` (ex. `["*"]` ou une liste de domaines) et `browser_blocked_domains`. Voir [39_browser_automation.md](39_browser_automation.md).
