...
| Terminal backends | local, docker, ssh, modal, … | local, `run_in_container`, docker services, `GET /api/terminal/capabilities`, **HTTP PTY** `/api/terminal/pty/sessions` (`portable-pty`), `akasha terminal capabilities` | Partiel | Moyenne | `../../43_session_terminal.md`, `../runtime/terminal-backends-roadmap.md` |
...
| Hooks | gateway / plugin / shell | `lifecycle_hooks.json` (`on_schedule_fire`, **`on_http_request_pre` / `on_http_request_post`** sur le trafic API), mode sandbox `strict` (allowlist commandes) | Partiel | Moyenne | `../runtime/gateway-shell-hooks.md`, `../../56_lifecycle_hooks.example.json` |
...
| Perf / SLO | Dashboard local (Hermes récent) | Métriques routeur, `scripts/bench-e2e.ps1` | Partiel | Moyenne | Runbook `../ops/slo-akasha-internal.md` |
| Cache idempotent | — | LRU **GET** `GET /api/router/models`, `GET /api/router/routes`, `GET /api/mcp/status` derrière `AKASHA_HTTP_CACHE_TTL_SECS` + plafond 256 / 256 Ko | Partiel | Moyenne | `../runtime/cache-strategy.md`, module `http_get_cache` |
| Git worktree | — | `git_*` outils + `akasha worktree list|add|remove|doctor` | Partiel | Moyenne | Wrapper git local + diagnostics opérateur + init projet Studio sur branche `main` et garde-fou reprise (`main`/`master`) |
...
| **`akasha-code-studio`** | Cockpit dev structuré : scheduler actionnable, `task_runs`, process watch, terminal, tools effective, MCP, lifecycle (+ fallback raw JSON) |
