...
| L3 | Découpage `api.rs` par domaines (tasks, config, memory, …) | En cours (lots A–F : `api_security`, `api_routes_*`) |
| L4 | `api_studio` / `api_workspace_graph` — alignement HTTP + services | En cours (`api_studio/mod.rs` + `handlers.rs`, `workspace_graph_handlers.rs`) |
...
- `crates/akasha-daemon/src/api_workspace_graph.rs` + `workspace_graph_handlers.rs`
...
| 2026-04-27 | ~17000 | Suite phase 1-3 : CSRF/query → `api_security.rs` ; automation / terminal / MCP / mission / profils → `api_routes_*.rs` ; cache agent → `agent_profile.rs` ; studio → `api_studio/` ; workspace graph → `workspace_graph_handlers.rs` (`wc -l` / PowerShell `Measure-Object -Line` sur les fichiers sources) |
