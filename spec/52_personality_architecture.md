...
| Fichier | Contenu |
|---------|--------|
| `spec/personality_core.yaml` | Noyau identitaire, valeurs, traits par défaut, règles comportementales, maniérismes (contenu rédigé en **anglais** dans le dépôt pour le bloc système) |
...
| `data_dir/agent_profile.json` | Surcharge utilisateur : nom, genre, formality (formal/informal), avatar, personnalité texte, rôle, rules, can_do, cannot_do, traits_override, preferred_mode |
...
1. **Identité** — Nom et genre (et formality) depuis le profil utilisateur en tête du bloc ; le noyau YAML (mission, posture, relation) complète seulement si l’utilisateur n’a pas défini de texte de personnalité, pour éviter les contradictions avec l’UI.
...
3. **Bloc personnalité** : champs profil utilisateur d’abord (nom, pronoms, formality, rôle, personnalité, rules/can/cannot), puis mission/posture/relation YAML si besoin, puis valeurs, traits, behavior_rules, maniérismes, mode actif, initiative, social/reasoning
