...
**Still roadmap / deeper work:** PTY persistence / transcript / agent tool `terminal_session` branché API, HTTP/SSE MCP client + OAuth in-product, **vrai** sandbox ressources pour hooks, plugin-level WASM hook **delivery** (événements côté host), rate limit distribué, cache GET étendu + invalidations fines, full catalog trust automation in `Akasha_plugins`.
